...
const CATEGORY_INFO = {
  name: "Électroménager",
  description: "Appareils pratiques pour la maison et la cuisine pour faciliter votre quotidien.",
  icon: "🍳",
  color: "orange"
};
...
function ProductCard({ product, onAddToCart }: { product: Product; onAddToCart: (p: Product) => void }) {
  ...
  const imageUrl = product.images?.[0] || `https://placehold.co/400x300/f97316/ffedd5?text=${encodeURIComponent(product.name)}`;
    ...
    <div className="product-card bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col group">
      <div className="relative overflow-hidden bg-gray-50">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1">
          {isNew && <span className="bg-orange-600 text-white text-xs font-bold px-2 py-1 rounded-full">NOUVEAU</span>}
      ...
      <div className="p-4 flex flex-col flex-1">
        <p className="text-xs text-orange-500 font-semibold uppercase tracking-wider mb-1">Électroménager</p>
        ...
        <button
          ...
          className="btn-primary w-full bg-orange-600 hover:bg-orange-700 text-white py-2.5 rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2"
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    fetch(`${API_URL}/products`)
      ...
      .then((data) => {
        const filtered = Array.isArray(data) 
          ? data.filter((p: Product) => p.categoryId?.toLowerCase().includes('electromenager'))
          : [];
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-orange-600 to-amber-600 text-white py-12">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <p className="text-orange-100 text-lg max-w-2xl">{CATEGORY_INFO.description}</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          ...
          <select
            ...
            className="border border-gray-200 rounded-xl px-4 py-2 text-sm font-medium text-gray-600 outline-none focus:border-orange-400 bg-white"
          >
        ...
        {loading ? (
          <div className="flex justify-center py-20">
            <LoadingSpinner size={60} color="#f97316" />
          </div>
        ) : products.length === 0 ? (
          <div className="text-center py-24 bg-white rounded-2xl border border-gray-100">
            ...
            <Link href="/" className="bg-orange-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-orange-700 transition-colors">
              Retour à l'accueil
            </Link>
          </div>
        ) : (
