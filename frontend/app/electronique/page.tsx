...
const CATEGORY_INFO = {
  name: "Électronique",
  description: "Power Banks, câbles, chargeurs et audio performants pour tous vos besoins technologiques.",
  icon: "⚡",
  color: "indigo"
};
...
function ProductCard({ product, onAddToCart }: { product: Product; onAddToCart: (p: Product) => void }) {
  ...
  const imageUrl = product.images?.[0] || `https://placehold.co/400x300/1e3a8a/4ade80?text=${encodeURIComponent(product.name)}`;
  ...
  return (
    <div className="product-card bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col group">
      ...
      <div className="p-4 flex flex-col flex-1">
        <p className="text-xs text-indigo-500 font-semibold uppercase tracking-wider mb-1">Électronique</p>
    ...
    fetch(`${API_URL}/products`)
      ...
      .then((data) => {
        const filtered = Array.isArray(data) 
          ? data.filter((p: Product) => p.categoryId?.toLowerCase().includes('electronique'))
          : [];
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-12">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <p className="text-indigo-100 text-lg max-w-2xl">{CATEGORY_INFO.description}</p>
        </div>
      </div>
